...
for (my $run = 1; $run <= $runs; $run++) {
    ...
    while (1) {
        ...
        if ($last_result > $max_keypress_force) {
            ...
            $deepest_location = $location_counter;
            last;
        }
    }
    sleep(1);
...
probe_for_switch_top();
warn "# fast downstroke\n";
move_gantry_z(0 - (($step * $deepest_location)));
my @down = record_force_data();
move_gantry_z(($step * $deepest_location));
my @up = record_force_data();
...
sub save_output {
    ...
    open(my $fast, ">", "force-tests/$maker/$type/fast-down.raw");

    print $fast join("\n", @down);
    close($fast);
    open(my $fast_up, ">", "force-tests/$maker/$type/fast_up.raw");
...
sub record_force_data {
    ...
    for (my $i = 0; $i <= 500; $i++) {
        ...
        if ($count_in) {
            ...
            while ($#bytes >= 0) {
                my $byte = shift @bytes;
                print STDERR ord($byte) . "\n";
                push @raw_data, ord($byte);
            }
        }
...
sub extract_base256_force_value {      #ds2-5n
    my @data  = @_;
    my $value = (((ord($data[4]) + (ord($data[3]) * 256)) / 1000) + (ord($data[2])) + (ord($data[1]) * 256));
    if (ord($data[5]) == 0x5C) {
        $value = 0 - $value;
    }
    return $value;
